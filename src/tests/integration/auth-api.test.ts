...
function createTestRequest(body: RegisterBody | LoginBody | Record<string, never>): NextRequest {
...
describe('認証API', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('必須項目が不足している場合はエラーを返す', async () => {
      const invalidRequests = [
        ...
        { email: 'test@example.com', password: 'password123' } as Partial<RegisterBody>,
      ]
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('間違ったパスワードではログインできない', async () => {
      ...
      expect(data.error).toMatch(/メールアドレスまたはパスワードが正しくありません/)
    })
